...
class YarnContainerLogReader(logFilePath: Path) {
  ...
  def read(): PeekingIterator[LogLine] = {
    ...
    if (!pathString.endsWith(TMP_FILE_SUFFIX) && fs.getFileStatus(logFilePath).getLen > 0L) {
      ...
      new PeekingIterator[LogLine](new YarnContainerLogIterator(reader, valueStream, logFilePath, logEventInfo))
    } else {
...
class YarnContainerLogIterator(
  ...
  logFilePath: Path,
  var logEventInfo: LogEventInfo) extends java.util.Iterator[LogLine] {
      ...
      try {
        if (len == -1 || curRead >= fileLength) {
          ...
          if (valueStream != null) {
            ...
            while (logEventInfo.sourceType != "gc" && toRead > 0) {
